...
def test_donor_A_relapse_periods(example_drug_tests_df):
    ...
    assert (
        donor_a.loc[0, "EVENT_DURATION"]
        == (pd.Timestamp("2024-01-10") - pd.Timestamp("2024-01-03")).days
    )
...
def test_donor_B_relapse_free_period(example_drug_tests_df):
    ...
    assert (
        donor_b.loc[0, "EVENT_DURATION"]
        == (pd.Timestamp("2024-02-04") - pd.Timestamp("2024-02-01")).days
    )
...
def test_donor_C_relapse_periods(example_drug_tests_df):
    ...
    assert (
        donor_c.loc[0, "EVENT_DURATION"]
        == (pd.Timestamp("2024-03-10") - pd.Timestamp("2024-03-07")).days
    )
...
def test_donor_D_relapse_periods(example_drug_tests_df):
    ...
    assert (
        donor_d.loc[0, "EVENT_DURATION"]
        == (pd.Timestamp("2024-07-02") - pd.Timestamp("2024-06-05")).days
    )
    ...
    assert (
        donor_d.loc[1, "EVENT_DURATION"]
        == (pd.Timestamp("2024-07-10") - pd.Timestamp("2024-07-05")).days
    )
...
def test_donor_F_relapse_periods(example_drug_tests_df):
    ...
    assert (
        donor_f.loc[0, "EVENT_DURATION"]
        == (pd.Timestamp("2024-06-10") - pd.Timestamp("2024-06-04")).days
    )
...
def sample_drug_tests_and_relapses_for_historical_split():
    ...
    drug_tests_df = pd.DataFrame(
        {"DONOR_ID": [1, 1, 1, 2, 2], "TIME": [50, 120, 220, 250, 350]}
    )
    return relapses_df, drug_tests_df


def test_define_historical_drug_tests_and_relapses(
    sample_drug_tests_and_relapses_for_historical_split,
):
    preprocessor = DataPreprocessorDrugRelapse({})
    relapses_df, drug_tests_df = sample_drug_tests_and_relapses_for_historical_split
    drug_tests_df_result, relapses_df_result = (
        preprocessor._define_historical_drug_tests_and_relapses(
            relapses_df.copy(), drug_tests_df.copy()
        )
    )
...
def test_define_historical_events():
    ...
    preprocessor = DataPreprocessorDrugRelapse({})
    updated_tests, updated_relapses = (
        preprocessor._define_historical_drug_tests_and_relapses(
            relapses_df, drug_tests_df
        )
    )
...
def test_get_info_positive_before_relapse_start(relapse_positive_merge_data):
    ...
    result = preprocessor._get_info_positive_before_relapse_start(
        relapses_df, drug_tests_df
    )
    ...
    assert (
        result.loc[result["RELAPSE_ID"] == 1, "PREV_POSITIVE_DRUGS"].iloc[0] == "heroin"
    )
    assert result.loc[result["RELAPSE_ID"] == 1, "PREV_NUM_DRUGS_POSITIVE"].iloc[0] == 2
    assert (
        result.loc[result["RELAPSE_ID"] == 2, "PREV_POSITIVE_DRUGS"].iloc[0]
        == "cocaine"
    )
