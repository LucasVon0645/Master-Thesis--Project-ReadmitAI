...
def fine_tune_after_head_training(
    ...
    model.train()  # ensure in train mode after loading
...
def main(
    ...
    model_filename: str = "fine_tuned_model",
):
    ...
    _, test_dataset, last_events_test_dataset, _, _ = prepare_datasets(
        ...
        # Raw filenames:
        raw_train_filename="train_more_prev_hosp_full.csv",
        raw_eval_filename="test_more_prev_hosp.csv",
        # Desired .pt filenames:
        train_pt_name="train_more_prev_hosp_full_dataset.pt",
        eval_pt_name="test_more_prev_hosp_dataset.pt",
        cache_pytorch_datasets_path=cache_pytorch_datasets_path,
        # Last-events:
        need_last_events_eval=True,
        last_events_pt_name="last_events_more_prev_hosp_dataset.pt",
        # Options:
    ...
    train_dataset, val_dataset, _, _, _ = prepare_datasets(
        ...
        model_config=model_config,
        # Raw filenames:
        raw_train_filename="train_more_prev_hosp_final.csv",
        raw_eval_filename="validation_more_prev_hosp_final.csv",
        # Desired .pt filenames:
        train_pt_name="train_more_prev_hosp_final_dataset.pt",
        eval_pt_name="validation_more_prev_hosp_final_dataset.pt",
        cache_pytorch_datasets_path=cache_pytorch_datasets_path,
        # Options:
        save_scaler_dir_path=None,
        overwrite_preprocessed=overwrite_data_files,
    ...
    # Train model
    model, loss_epochs = fine_tune_after_head_training(
        train_dataset, model_config, model_pretrained_path,
        neptune_run=neptune_run, writer=writer,
...
if __name__ == "__main__":
    ...
    if not os.path.exists(model_pretrained_path):
        raise FileNotFoundError(f"Model pretrained path does not exist: {model_pretrained_path}")
